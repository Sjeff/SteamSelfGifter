...
describe("giveawayFiltersStore", () => {
  ...
  it("merges partial updates", () => {
    ...
    useGiveawayFiltersStore
      .getState()
      .setFilters({ minChance: 0.5, endingWithin: 6 });
  ...
  it("persists filters to localStorage", () => {
    useGiveawayFiltersStore
      .getState()
      .setFilters({ minScore: 8, status: "wishlist" });
  ...
  it("does not persist the search query", () => {
    useGiveawayFiltersStore
      .getState()
      .setFilters({ search: "portal", minScore: 5 });
  ...
  it("resetFilters restores defaults", () => {
    useGiveawayFiltersStore
      .getState()
      .setFilters({ minScore: 9, minChance: 10 });
    useGiveawayFiltersStore.getState().resetFilters();
