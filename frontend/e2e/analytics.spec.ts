...
test.describe("Analytics page", () => {
  test("renders trend charts with legend and table view", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/analytics-light.png",
      fullPage: true,
    });

    // Dark mode render. index.css transitions background-color/color over
    // 0.2s, so wait it out before the screenshot or it captures a
    // half-faded mix of both themes.
