...
test.describe("Dashboard", () => {
  ...
  test("stopping the scheduler calls the stop endpoint", async ({ page }) => {
    ...
    // AccountSwitcher auto-selects the default account, so this may hit
    // either the flat or the per-account scheduler path.
    await expect
      .poll(() =>
        calls.some(
          (c) =>
            c.method === "POST" &&
            /\/(scheduler|accounts\/\d+\/scheduler)\/stop$/.test(c.url),
        ),
      )
      .toBe(true);
  });
